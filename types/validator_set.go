...
// SelectProposerByVotingPower selects a proposer based on voting power and a seed.
// This is more weighted random selection, giving validators with higher voting power
// more chance to be selected as a proposer.
func (vals *ValidatorSet) SelectProposerByVotingPower(seed int64) *Validator {
	if vals.IsNilOrEmpty() {
		panic("empty validator set")
	}
	...
	// Compute a weighted random selection threshold
	threshold := (seed % totalPower)
	powerSum := int64(0)

	// Deterministically select validator based on voting power
	for _, val := range vals.Validators {
		powerSum += val.VotingPower
		if powerSum > threshold {
			return val
		}
	}

	// Fallback - should never reach here if calculations are correct
	return vals.Validators[0]
}

// SelectProposerWeighted selects a proposer using a combination of voting power and
// rotation pattern to ensure fairness while maintaining security.
// It uses the block height as part of the seed for deterministic selection.
func (vals *ValidatorSet) SelectProposerWeighted(height, round int64) *Validator {
	if vals.IsNilOrEmpty() {
		panic("empty validator set")
	}

	// Create a deterministic seed from height and round
	seed := height + (round * 1000)

	// Use different strategies depending on validator set size
	if len(vals.Validators) <= 4 {
		// For small validator sets, use round-robin selection
		index := int(seed % int64(len(vals.Validators)))
		return vals.Validators[index]
	} else {
		// For larger sets, use weighted selection
		// First, create a deterministic permutation based on seed
		permutation := vals.deterministicPermutation(seed)

		// Then use weighted selection among the top half of validators
		// This maintains security while allowing for reasonable decentralization
		topCount := (len(vals.Validators) + 1) / 2
		topVals := make([]*Validator, topCount)
		for i := 0; i < topCount; i++ {
			topVals[i] = vals.Validators[permutation[i]]
		}

		// Create a sub-validator set from the top validators
		subSet := NewValidatorSet(topVals)

		// Use weighted selection from the subset
		return subSet.SelectProposerByVotingPower(seed)
	}
}

// deterministicPermutation returns a deterministic permutation of validator indices
// based on the given seed.
func (vals *ValidatorSet) deterministicPermutation(seed int64) []int {
	// Create a list of indices
	indices := make([]int, len(vals.Validators))
	for i := range indices {
		indices[i] = i
	}
